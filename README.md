...
这是一个面向上海科技大学校园场景的微信小程序 MVP，用于失物招领、寻物发布、校内地点定位和相似物品提醒。当前版本默认使用本地 mock 数据，无需云环境即可在微信开发者工具里预览核心流程。
...
当前定位链路使用普通微信定位 + 校内地点库：

1. 连续调用 3 次微信高精度定位，选取 `距离校内 POI + 定位精度` 综合分最低的一次。
2. 只根据 GPS/GCJ-02 坐标与上科大校内地点库计算最近地点。
3. 当定位精度足够、最近地点足够近时自动填充；否则展示候选地点，要求用户确认，避免误判。
...
当前版本只调用 `wx.getLocation` 获取用户当前位置，用于推荐附近校内地点。不采集 Wi-Fi、蓝牙、设备信标或腾讯地图室内定位信号。
...
- 正式上线前需要配置真实 AppID、云开发环境，并在微信后台完成位置权限相关隐私声明。
